...
public class ParkingCarIn extends JOptionPane implements ActionListener {
	...
	//각 '주차공간'버튼 클릭 시 발생한 이벤트를 처리하는 핸들러
	@Override
	...
	class parkOkListener implements ActionListener //'주차등록' 클릭 시
	{
		@Override
		public void actionPerformed(ActionEvent e) {
			// TODO Auto-generated method stub
			...
			SimpleDateFormat format1 = new SimpleDateFormat("hh시 mm분 ss초"); //시간을 저장할 틀
			String time = format1.format(presentTime.getTime()); //'주차내역'테이블에 출력하기 위해 변환해서 저장
			
			String cKind = ""; //라디오버튼에 있는 텍스트(소형차,중형차)를 담기 위한 변수
			if (car1Box.getState() == true)
			{
				cKind = car1Box.getLabel(); //"소형차" 스트링 값 가져오기
			}
			...
			if (find == true) //List안에 입력받은 번호과 같은 번호가 있으면 텍스트필드에 다시 입력받게 한다.
			{
				...
				inputNumText.setText(""); //return되서 오면 다시 입력받기 위해 텍스트필드를 초기화한다.
				inputNumText.requestFocus(); //
				return; //parkOkListener의 처음으로 돌아간다.
			}
			else //List안에 입력받은 번호과 같은 번호가 없으면 주차등록이 가능하게 된다!
			{
				parkCarList.add(new ParkCarInfo(parkNum, cKind, inputNumText.getText(), carIntime)); //주차공간번호, 차량종류, 입력받은 차량번호, 주차공간버튼On설정, 주차차량 입고설정
				...
				parkButton.setText("주차중"); //주차 등록이 완료되면 그 주차공간은 '주차중'이라는 표시를 하게함.
